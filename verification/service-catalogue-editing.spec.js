...
(async () => {
  ...
  await page.goto(`${ORIGIN}/services/`, { waitUntil: "load" });
  ...
  await page.evaluate(() => window.renderServiceCatalogue && window.renderServiceCatalogue());
  await page.waitForTimeout(600);

  const selects = await page.$$eval(".service-status-select", (els) => els.map((el) => ({
    id: el.dataset.serviceId, value: el.value, current: el.dataset.current,
    options: [...el.options].map((o) => `${o.value}:${o.textContent}`),
    disabled: el.disabled,
  })));
  ok("a status control renders on every row", selects.length === 2, `${selects.length} selects`);
  ok("all three states are offered", selects[0] && selects[0].options.length === 3,
    selects[0] ? selects[0].options.join(" | ") : "none");

  // THE GATED ROW SHOWS WHAT IS STORED, NOT WHAT IS SERVED. Showing the served
  // status would make it un-settable: it would snap back to coming soon on
  // every render and the operator could never express "I want this live".
  const gated = selects.find((s) => s.id.startsWith("1111"));
  ok("the gated row shows its STORED status, not the gated one",
    gated && gated.value === "active", gated ? gated.value : "missing");

  const plain = selects.find((s) => s.id.startsWith("2222"));
  ok("an ungated row shows its own status", plain && plain.value === "disabled",
    plain ? plain.value : "missing");

  // Changing the ungated row: the request, and the confirmation.
  putBodies = [];
  await page.selectOption('.service-status-select[data-service-id^="2222"]', "active");
  await page.waitForTimeout(900);
  ok("changing it sends one PUT for that service only", putBodies.length === 1,
    JSON.stringify(putBodies));
  ok("it sends only the status", putBodies[0] && JSON.stringify(putBodies[0].body) === '{"status":"active"}',
    putBodies[0] ? JSON.stringify(putBodies[0].body) : "none");
  const plainToast = await page.evaluate(() => document.querySelector(".toast, #toast, [data-toast]")?.textContent?.trim() || "");
  ok("and says plainly that it is live", /Events is now Live/i.test(plainToast), plainToast || "(no toast)");

  // THE CASE THIS HARNESS EXISTS FOR: the answer differs from the request.
  await page.evaluate(() => window.renderServiceCatalogue && window.renderServiceCatalogue());
  await page.waitForTimeout(600);
  putBodies = [];
  await page.selectOption('.service-status-select[data-service-id^="1111"]', "coming_soon");
  await page.waitForTimeout(300);
  await page.evaluate(() => window.renderServiceCatalogue && window.renderServiceCatalogue());
  await page.waitForTimeout(400);
  await page.selectOption('.service-status-select[data-service-id^="1111"]', "active");
  await page.waitForTimeout(900);
  const gatedToast = await page.evaluate(() => document.querySelector(".toast, #toast, [data-toast]")?.textContent?.trim() || "");
  ok("a gated save is reported honestly, not as success",
    /stored as Live/i.test(gatedToast) && /customers still see Coming soon/i.test(gatedToast),
    gatedToast || "(no toast)");

  // The page must not still claim there is nothing to switch on.
  const text = await page.evaluate(() => document.body.innerText);
  ok("the page no longer says there is nothing to switch on",
    !/nothing to switch on here/i.test(text));
  ok("and it explains what setting a gated row live will not do",
    /customers keep seeing .coming soon./i.test(text));

  ok("no page errors", errors.length === 0, errors.slice(0, 3).join(" | "));

  await page.screenshot({ path: "/tmp/claude-0/-home-user-titopay-app/b46bca12-b8d1-59fa-a0f4-cf119e42703a/scratchpad/catalogue.png", fullPage: true });
  await browser.close();
  server.close();
  console.log(bad ? `\n${bad} check(s) failed` : "\nall checks passed");
