...
async function createChallenge(user,event,meta={},options={}) {
  ...
  const names=String(user.full_name||"").trim().split(/\s+/);
  await queueEmail({recipient:user.email,templateKey:"email_otp",userId:userType==="customer"?user.id:null,variables:{firstName:names[0]||"there",fullName:user.full_name,email:user.email,otp:code},idempotencyKey:`email-otp:${challengeId}`,metadata:{otpChallengeId:challengeId,purpose,...(options.metadata||{})}});
