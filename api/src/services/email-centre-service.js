...
// The wordmark travels INSIDE the message as an inline CID attachment, so a
// mail client that blocks remote images (most webmail defaults) still shows
// the logo - it is part of the email, not a remote fetch. Providers whose
// HTTP APIs cannot carry inline images fall back to the hosted URL at
// delivery time, and if this file is ever missing the header falls back too.
const EMAIL_LOGO_CID = "titopay-wordmark";
let EMAIL_LOGO_BASE64 = "";
try {
  EMAIL_LOGO_BASE64 = fs.readFileSync(path.join(__dirname, "..", "assets", "email-logo.png")).toString("base64");
} catch (_error) {
  // Header renders from TITOPAY_LOGO_URL instead.
}
const EMAIL_LOGO_SRC = () => (EMAIL_LOGO_BASE64 ? `cid:${EMAIL_LOGO_CID}` : TITOPAY_LOGO_URL);

// Attach the inline wordmark for providers that can carry it; swap the cid
// for the hosted URL for providers that cannot. Never both, never neither.
function contentForDelivery(content, inlineCapable) {
  const html = String(content.html || "");
  if (!html.includes(`cid:${EMAIL_LOGO_CID}`)) return content;
  if (inlineCapable && EMAIL_LOGO_BASE64) {
    return { ...content, attachments: [...(content.attachments || []),
      { filename: "titopay.png", contentBase64: EMAIL_LOGO_BASE64, contentType: "image/png", cid: EMAIL_LOGO_CID }] };
  }
  return { ...content, html: html.split(`cid:${EMAIL_LOGO_CID}`).join(TITOPAY_LOGO_URL) };
}
...
async function dashboard(allowedTemplates=null){await ensureEmailSchema();const scoped=allowedTemplates?"template_key=ANY($1::text[])":"TRUE",params=allowedTemplates?[allowedTemplates]:[];const [metrics,perDay,deliveryOutcomes,byTemplate,queueActivity,recent]=await Promise.all([
  pool.query(`SELECT COUNT(*) FILTER(WHERE (sent_at AT TIME ZONE 'Africa/Johannesburg')::date=(NOW() AT TIME ZONE 'Africa/Johannesburg')::date)::int sent_today,COUNT(*) FILTER(WHERE (delivered_at AT TIME ZONE 'Africa/Johannesburg')::date=(NOW() AT TIME ZONE 'Africa/Johannesburg')::date)::int delivered_today,COUNT(*) FILTER(WHERE (failed_at AT TIME ZONE 'Africa/Johannesburg')::date=(NOW() AT TIME ZONE 'Africa/Johannesburg')::date)::int failed_today,COUNT(*) FILTER(WHERE status IN ('queued','processing'))::int queued,COALESCE(ROUND(100.0*COUNT(*) FILTER(WHERE delivered_at IS NOT NULL)/NULLIF(COUNT(*) FILTER(WHERE sent_at IS NOT NULL),0),2),0) delivery_rate,COALESCE(ROUND((AVG(EXTRACT(EPOCH FROM(delivered_at-sent_at))) FILTER(WHERE delivered_at IS NOT NULL))::numeric,0),0) average_delivery_seconds FROM email_queue WHERE ${scoped}`,params),
  pool.query(`SELECT (created_at AT TIME ZONE 'Africa/Johannesburg')::date AS "day",COUNT(*)::int AS sent,COUNT(*) FILTER(WHERE status='delivered')::int AS delivered,COUNT(*) FILTER(WHERE status IN ('failed','dead_lettered'))::int AS failed FROM email_queue WHERE ${scoped} AND created_at>=((NOW() AT TIME ZONE 'Africa/Johannesburg')::date-INTERVAL '29 days') GROUP BY 1 ORDER BY 1`,params),
  pool.query(`SELECT CASE WHEN status='delivered' THEN 'successful' ELSE 'failed' END outcome,COUNT(*)::int count FROM email_queue WHERE ${scoped} AND created_at>=((NOW() AT TIME ZONE 'Africa/Johannesburg')::date-INTERVAL '29 days') AND status IN ('delivered','failed','dead_lettered') GROUP BY 1 ORDER BY 1`,params),
  pool.query(`SELECT template_key,COUNT(*)::int count FROM email_queue WHERE ${scoped} AND created_at>=((NOW() AT TIME ZONE 'Africa/Johannesburg')::date-INTERVAL '29 days') GROUP BY 1 ORDER BY 2 DESC LIMIT 20`,params),
...
async function analytics(query={}, allowedTemplates=null) {
  ...
  const rollup=(bucket,rangeStart)=>pool.query(`SELECT DATE_TRUNC('${bucket}',(q.created_at AT TIME ZONE 'Africa/Johannesburg')) period,
...
async function deliverQueuedContent(job){const provider=await effectiveProvider(job.provider);
  ...
  const content=contentForDelivery(JSON.parse(decrypt(job.encrypted_content)),["smtp","ses","postmark","sendgrid"].includes(provider.provider));if(provider.provider==="smtp"||(provider.provider==="ses"&&provider.smtpHost)){if(!provider.smtpHost)throw new Error("SMTP email provider is not configured");const transport=nodemailer.createTransport({host:provider.smtpHost,port:provider.smtpPort,secure:Boolean(provider.smtpSecure),auth:provider.smtpUser||provider.smtpPassword?{user:provider.smtpUser,pass:provider.smtpPassword}:undefined,tls:{rejectUnauthorized:config.integrations.email.smtpRejectUnauthorized}});// A verification code is disposable mail, and the message can say so:
...
const mailAttachments=(content.attachments||[]).map((item)=>({filename:item.filename,content:Buffer.from(item.contentBase64,"base64"),contentType:item.contentType,...(item.cid?{cid:item.cid,contentDisposition:"inline"}:{})}));
...
async function claimJobs(workerId,limit=5){await ensureEmailSchema();await pool.query(`UPDATE email_queue SET status='queued',locked_at=NULL,locked_by=NULL WHERE status='processing' AND locked_at<NOW()-INTERVAL '15 minutes'`);const client=await pool.connect();try{await client.query("BEGIN");const settings=(await client.query("SELECT daily_sending_limit,sending_enabled FROM email_settings WHERE id=TRUE FOR UPDATE")).rows[0];if(!settings?.sending_enabled){await client.query("COMMIT");return [];}const usage=(await client.query(`SELECT COUNT(*) FILTER(WHERE sent_at>=CURRENT_DATE)::int sent_today,COUNT(*) FILTER(WHERE status='processing')::int processing FROM email_queue`)).rows[0];const remaining=Math.max(0,Number(settings.daily_sending_limit)-Number(usage.sent_today)-Number(usage.processing));const claimLimit=Math.min(Math.max(1,Number(limit)||1),remaining);if(!claimLimit){await client.query("COMMIT");return [];}const {rows}=await client.query(`SELECT * FROM email_queue WHERE status='queued' AND scheduled_at<=NOW() ORDER BY (CASE WHEN template_key IN ('email_otp','password_change_otp') THEN 0 ELSE 1 END),scheduled_at,created_at FOR UPDATE SKIP LOCKED LIMIT $1`,[claimLimit]);if(rows.length)await client.query(`UPDATE email_queue SET status='processing',locked_at=NOW(),locked_by=$2 WHERE id=ANY($1::uuid[])`,[rows.map(r=>r.id),workerId]);await client.query("COMMIT");return rows.map(r=>({...r,status:"processing"}));}catch(error){await client.query("ROLLBACK");throw error;}finally{client.release();}}
