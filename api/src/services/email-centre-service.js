...
const DEFAULT_TEMPLATES = [
  ...
  ["hr_announcement", "HR Announcement", "{{companyName}}: {{announcementTitle}}", "<p>Hello {{firstName}},</p><p><strong>{{announcementTitle}}</strong></p><p>{{announcementBody}}</p><p><a href=\"{{appUrl}}\" style=\"display:inline-block;padding:12px 20px;background:#168ac2;color:#fff;text-decoration:none;border-radius:6px\">Open the HR portal</a></p><p>This is an internal staff announcement from {{companyName}}.</p>"],
  ["hr_leave_decision", "HR Leave Decision", "Your {{leaveType}} leave request was {{decision}}", "<p>Hello {{firstName}},</p><p>Your <strong>{{leaveType}}</strong> leave from {{leaveStart}} to {{leaveEnd}} ({{leaveDays}} day(s)) was <strong>{{decision}}</strong>.</p><p>{{decisionComment}}</p><p><a href=\"{{appUrl}}\" style=\"display:inline-block;padding:12px 20px;background:#168ac2;color:#fff;text-decoration:none;border-radius:6px\">View it in the HR portal</a></p>"],
  ["hr_claim_decision", "HR Expense Claim Decision", "Your {{claimType}} claim is now {{decision}}", "<p>Hello {{firstName}},</p><p>Your <strong>{{claimType}}</strong> claim for {{currency}} {{claimAmount}} is now <strong>{{decision}}</strong>.</p><p><a href=\"{{appUrl}}\" style=\"display:inline-block;padding:12px 20px;background:#168ac2;color:#fff;text-decoration:none;border-radius:6px\">View it in the HR portal</a></p>"],
  ["hr_request_update", "HR Request Update", "Your HR request {{requestReference}} has an update", "<p>Hello {{firstName}},</p><p>There is an update on your request <strong>{{requestSubject}}</strong> (reference {{requestReference}}). It is now <strong>{{requestStatus}}</strong>.</p><p>The reply is in the HR portal rather than in this email, because internal requests can contain confidential information.</p><p><a href=\"{{appUrl}}\" style=\"display:inline-block;padding:12px 20px;background:#168ac2;color:#fff;text-decoration:none;border-radius:6px\">Read it in the HR portal</a></p>"],
  ["hr_onboarding_task", "HR Onboarding Task", "An onboarding task is waiting for you", "<p>Hello {{firstName}},</p><p><strong>{{taskTitle}}</strong> ({{taskCategory}}) has been added to your onboarding. Please complete it by {{taskDueDate}}.</p><p><a href=\"{{appUrl}}\" style=\"display:inline-block;padding:12px 20px;background:#168ac2;color:#fff;text-decoration:none;border-radius:6px\">Open your onboarding</a></p>"],
  ["hr_training_reminder", "HR Mandatory Training Reminder", "{{courseTitle}} is due", "<p>Hello {{firstName}},</p><p><strong>{{courseTitle}}</strong> is a mandatory course and was due on {{courseDueDate}}.</p><p><a href=\"{{appUrl}}\" style=\"display:inline-block;padding:12px 20px;background:#168ac2;color:#fff;text-decoration:none;border-radius:6px\">Complete it in the Learning Hub</a></p>"],
...
async function deliverQueuedContent(job){const provider=await effectiveProvider(job.provider);const content=JSON.parse(decrypt(job.encrypted_content));if(provider.provider==="smtp"||(provider.provider==="ses"&&provider.smtpHost)){if(!provider.smtpHost)throw new Error("SMTP email provider is not configured");const transport=nodemailer.createTransport({host:provider.smtpHost,port:provider.smtpPort,secure:Boolean(provider.smtpSecure),auth:provider.smtpUser||provider.smtpPassword?{user:provider.smtpUser,pass:provider.smtpPassword}:undefined,tls:{rejectUnauthorized:config.integrations.email.smtpRejectUnauthorized}});const result=await transport.sendMail({from:provider.from,replyTo:provider.replyTo,to:job.recipient,subject:job.subject,html:content.html,text:content.text});return {messageId:result.messageId,accepted:result.accepted,rejected:result.rejected,response:result.response};}
