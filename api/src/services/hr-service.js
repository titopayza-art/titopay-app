...
async function dashboard(auth) {
  const [employees, leave, attendance, payroll, onboarding, candidates, tickets, calendarRows, departmentRows, activityRows] = await Promise.all([
    ...
    safeDashboardQuery("SELECT COALESCE(SUM(CASE WHEN status = 'present' THEN 1 ELSE 0 END),0)::int present, COALESCE(SUM(CASE WHEN work_mode ILIKE 'remote%' THEN 1 ELSE 0 END),0)::int remote, COALESCE(SUM(CASE WHEN minutes_late > 0 THEN 1 ELSE 0 END),0)::int late, COALESCE(SUM(CASE WHEN status = 'absent' THEN 1 ELSE 0 END),0)::int absent FROM hr_attendance_records WHERE deleted_at IS NULL AND work_date::date = (NOW() AT TIME ZONE 'Africa/Johannesburg')::date", [{ present: 0, remote: 0, late: 0, absent: 0 }]),
    ...
    safeDashboardQuery("SELECT COALESCE(SUM(CASE WHEN status = 'complete' THEN 1 ELSE 0 END),0)::int complete, COALESCE(SUM(CASE WHEN status = 'in_progress' THEN 1 ELSE 0 END),0)::int in_progress, COALESCE(SUM(CASE WHEN due_date < (NOW() AT TIME ZONE 'Africa/Johannesburg')::date AND status <> 'complete' THEN 1 ELSE 0 END),0)::int overdue FROM hr_onboarding_tasks WHERE deleted_at IS NULL", [{ complete: 0, in_progress: 0, overdue: 0 }]),
    ...
    safeDashboardQuery("SELECT first_name, last_name, start_date FROM hr_employees WHERE deleted_at IS NULL AND status = 'active' AND start_date IS NOT NULL AND EXTRACT(MONTH FROM start_date) = EXTRACT(MONTH FROM (NOW() AT TIME ZONE 'Africa/Johannesburg')::date) LIMIT 8", []),
