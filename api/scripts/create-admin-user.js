...
async function main() {
  ...
  if (!ALLOWED_ROLES.has(role)) {
    throw new Error(`ADMIN_ROLE must be one of: ${Array.from(ALLOWED_ROLES).join(", ")}`);
  }
  ...
  try {
      ...
      result = await pool.query(
        `UPDATE admin_users
         SET full_name = $2,
             ...
             updated_at = NOW()
         WHERE id = ANY($1::uuid[])
         RETURNING id, full_name, username, email, role, status, updated_at`,
    ...
    await forceAdminPasswordOnlyPolicy("admin_create_script");
