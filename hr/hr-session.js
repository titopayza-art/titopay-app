...
(() => {
  ...
  // The production API authentication boundary changed while some Safari
  // installations retained the old user object and tokens. Clear that legacy
  // state once, before React reads it, so the portal cannot display a signed-in
  // shell while authenticated module requests use an invalid token.
  if (localStorage.getItem(SESSION_BUILD_KEY) !== SESSION_BUILD) {
    clearSession();
    localStorage.setItem(SESSION_BUILD_KEY, SESSION_BUILD);
  }
