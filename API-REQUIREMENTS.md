...
## P1-6 — Wallet passes for the customer's own payment and tip QR

**Screens:** QR poster (payment and tip). Shipped v271: the poster shows
"Add to Apple Wallet" on iOS/macOS and "Add to Google Wallet" on Android,
next to the Download A4 PDF button. Tapping it today reports honestly that
no pass has been issued yet; the moment the endpoints below answer, the
same tap saves a real pass — no client change.

The signing rule is the same as tickets (P0-3): Apple's Pass Type ID
certificate and the Google service-account key can only live on the server,
so the client never builds a pass — it links to one.

Two ways to supply each, either is enough:

1. Put a pass URL on the QR record `/v1/qr/generate-static` and
   `/v1/qr/profile` return — same field names as tickets
   (`appleWalletUrl` / `googleWalletUrl` / `samsungWalletUrl`, camel or
   snake case, plus the aliases listed in P0-3). The poster renders a
   direct link.
2. Serve lookups the client calls when the record has no URL:

```
GET /v1/qr/{qrId}/apple-wallet    → 200 { "appleWalletUrl": "…" }
GET /v1/qr/{qrId}/google-wallet   → 200 { "googleWalletUrl": "…" }
GET /v1/qr/{qrId}/samsung-wallet  → 200 { "samsungWalletUrl": "…" }
```

A `404` is read as "no pass issued yet" and reported as exactly that.
Samsung Wallet needs Samsung partner onboarding, so the client only shows
that control when the record already carries a Samsung URL — Google Wallet
covers all Android phones (Samsung included) in the meantime.

Suggested pass content (generic pass type on both platforms): account
holder or business name, the QR image as the barcode/QR payload, wallet ID,
and the TitoPay brand colours. The pass is a way to present the same QR
from the phone's lock screen — it grants nothing and carries no balance.
