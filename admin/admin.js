...
async function renderIntegrations(me = {}) {
  ...
  const allProviders = configResult.providers || [];
  // A capability that belongs to a parent provider (Peach Payout) is shown
  // inside that parent's card rather than as a card of its own, so the list
  // still reads as one Peach Payments integration.
  const groupedCapabilities = allProviders.filter((item) => item.peachGroup && item.peachGroup !== item.key);
  const providers = allProviders.filter((item) => !item.peachGroup || item.peachGroup === item.key);
  const capabilitiesFor = (key) => [
    ...(allProviders.find((item) => item.key === key && item.capability) ? [allProviders.find((item) => item.key === key)] : []),
    ...groupedCapabilities.filter((item) => item.peachGroup === key)
  ];
  const logs = logsResult.logs || [];
  const webhooks = webhooksResult.webhooks || [];
  const isSuperAdmin = hasFullAdminAccess(me);
  PAGE_EXPORTS.integrations = allProviders.map(({ secrets, ...row }) => row);
  ...
  document.getElementById("page-content").innerHTML = `
    ...
    ${renderIntegrationHealthDashboard(allProviders)}
          ...
          <div class="integration-card-header">
            ...
            ${(() => {
              ...
              if (capabilities.length > 1) {
                const overall = combinedCapabilityStatus(capabilities);
                const label = overall === "connected" ? "Connected"
                  : overall === "partially_configured" ? "Partially configured"
                  : overall === "failed" ? "Failed" : "Not configured";
                const tone = overall === "connected" ? "green" : overall === "partially_configured" ? "orange" : overall === "failed" ? "red" : "orange";
                return `<span class="chip ${tone}">${label}</span>`;
              }
              return `<span class="chip ${provider.enabled === false ? "red" : provider.configured ? "green" : "orange"}">${provider.enabled === false ? "Disabled" : provider.configured ? "Configured" : "Not configured"}</span>`;
            })()}
          </div>
          ${capabilitiesFor(provider.key).length > 1 ? capabilityStatusStrip(capabilitiesFor(provider.key)) : ""}
          <dl class="smtp-detail-grid">
...
function capabilityStatus(provider = {}) {
  const status = String(provider.health?.status || "").toLowerCase();
...
function capabilityStatusStrip(capabilities = []) {
  return `
    <div class="integration-status-strip">
      ${capabilities.filter(Boolean).map((item) => `
        <span><strong>${escapeHtml(item.capabilityLabel || item.label)}</strong>
          <span class="chip ${integrationStatusClass(capabilityStatus(item))}">${escapeHtml(integrationStatusLabel(capabilityStatus(item)))}</span>
        </span>
      `).join("")}
    </div>
  `;
...
function integrationCapabilityCards(provider, providerKey, isSuperAdmin, heading) {
  const title = heading || "Configuration";
  return `
    ${tableCard(title, `
    ...
    ${tableCard(`${title} — Operational Status`, `
      <dl class="smtp-detail-grid">
        ...
        <div><dt>Base URL</dt><dd>${escapeHtml(provider.baseUrl || provider.defaultBaseUrl || "Not configured")}</dd></div>
...
function bindIntegrationForms(me) {
  ...
  document.querySelectorAll(".integration-form").forEach((form) => {
    form.addEventListener("submit", async (event) => {
      ...
      body.enabled = formData.get("enabled") === "on";
      try {
        await apiFetch(`/admin/integrations/${targetKey}`, {
          method: "PUT",
          body: JSON.stringify(body),
        });
        showToast("Integration configuration saved");
...
async function renderIntegrationProvider(me = {}) {
  ...
  const [providerResult, companionResult, routingResult] = await Promise.all([
    apiFetch(`/admin/integrations/config/${providerKey}`),
    companionKey ? apiFetch(`/admin/integrations/config/${companionKey}`).catch(() => ({ provider: null })) : Promise.resolve({ provider: null }),
    apiFetch("/admin/provider-routing").catch(() => ({ services: [], providers: [] })),
  ]);
  const provider = providerResult.provider;
  const companion = companionResult.provider;
  const isSuperAdmin = hasFullAdminAccess(me);
  PAGE_EXPORTS["integration-provider"] = [provider, companion]
    .filter(Boolean)
    .map((item) => ({ ...item, secrets: undefined }));
  const poweredServices = (routingResult.services || []).filter((service) => service.provider === providerKey);
  const capabilities = [provider, companion].filter(Boolean);
  const overall = companion ? combinedCapabilityStatus(capabilities) : (provider.health?.status || "not_tested");
  const collectionHeading = companion ? (provider.capabilityLabel || "Collection / Top-up") : "Configuration";
  ...
  document.getElementById("page-content").innerHTML = `
    ...
    <section class="panel-grid">
      ${integrationCapabilityCards(provider, providerKey, isSuperAdmin, collectionHeading)}
    </section>
    ${companion ? `
      <section class="panel-grid">
